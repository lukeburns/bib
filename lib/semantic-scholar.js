...
class SemanticScholarAPI {
  ...
  async rateLimit(skipIfRecentlyCached = false) {
    // Skip rate limiting if we recently served from cache
    if (skipIfRecentlyCached && this.recentlyCachedRequest) {
      this.recentlyCachedRequest = false;
      return;
    }
  ...
  async makeRequest(path, retryCount = 0) {
    ...
    if (cachedResult !== null) {
      ...
      this.recentlyCachedRequest = true;
