...
class SemanticScholarAPI {
  ...
  async rateLimit(skipIfCached = false) {
  ...
  async makeRequest(path, retryCount = 0) {
    ...
    if (cachedResult !== null) {
      // Debug: Log slow cache operations
      if (cacheTime > 100 && !this.quiet) {
        console.log(`    Slow cache lookup: ${cacheTime}ms`);
      }
  ...
  async getPaperByDOI(doi) {
    // Check if this is an arXiv DOI and redirect to arXiv lookup
    const arxivMatch = doi.match(/10\.48550\/arXiv\.(\d{4}\.\d{4,5})/i);
    if (arxivMatch) {
      const arxivId = arxivMatch[1];
      if (!this.quiet) {
        console.log(`  Detected arXiv DOI, using arXiv ID: ${arxivId}`);
      }
      const path = `/graph/v1/paper/ARXIV:${arxivId}?fields=paperId,title,authors,year,citationCount,referenceCount,journal,externalIds,references,references.title,references.authors,references.year,references.citationCount,references.externalIds`;
      try {
        const result = await this.makeRequest(path);
        ...
        if (!this.quiet) console.warn(`arXiv lookup failed for ${arxivId}: ${error.message}`);
      }
      return null;
    }
  ...
  async makeRequestWithoutRateLimit(path, retryCount = 0) {
    // Check cache first
    const cacheStartTime = Date.now();
    const cachedResult = this.cache.get(path);
    const cacheTime = Date.now() - cacheStartTime;
    ...
    // If not cached, this is a real API call - update last request time but don't delay
    this.lastRequestTime = Date.now();
        ...
        res.on('end', async () => {
          if (res.statusCode === 200) {
            ...
            if (retryCount < this.maxRetries) {
              ...
              await new Promise(resolve => setTimeout(resolve, backoffTime));
              try {
                const result = await this.makeRequestWithoutRateLimit(path, retryCount + 1);
                resolve(result);
              } catch (error) {
                reject(error);
              }
            ...
            try {
              ...
              if (work.reference && work.reference.length > 0) {
                ...
                const references = work.reference.map(ref => {
                  // CrossRef can have different title fields, fallback to unstructured
                  let title = ref['article-title'] || ref.title || ref['volume-title'] || ref['chapter-title'];
                  
                  // If no proper title, use unstructured field
                  if (!title || title.length < 15) {
                    title = ref.unstructured || title;
                  }
                  
                  // Debug: Log ALL titles to find "et al" source
                  if (title && (title.toLowerCase().includes('et al') || title.toLowerCase().startsWith('and ') || title.length < 10)) {
                    console.log('DEBUG CrossRef suspicious title:', {
                      finalTitle: title,
                      'article-title': ref['article-title'],
                      'title': ref.title,
                      'volume-title': ref['volume-title'],
                      'chapter-title': ref['chapter-title'],
                      'unstructured': ref.unstructured,
                      DOI: ref.DOI,
                      fullRef: JSON.stringify(ref, null, 2).substring(0, 800)
                    });
                  }
  ...
  async findPaper(bibEntry) {
    ...
    if (identifiers.doi) {
      lastResult = await this.getPaperByDOI(identifiers.doi);
      if (lastResult) return lastResult;
    }

    // Try arXiv ID - use smart rate limiting if previous was cached
    if (identifiers.arxiv) {
      ...
      try {
        // Rate limit smartly based on previous cache hit
        await this.rateLimit(lastResult && lastResult.fromCache);
        const result = await this.makeRequestWithoutRateLimit(path);
        if (result && result.data) return { ...result.data, fromCache: result.fromCache };
      } catch (error) {
        if (!this.quiet) console.warn(`arXiv lookup failed: ${error.message}`);
      }
    }

    // Fallback to title search - use smart rate limiting
   ...
   */
  async getCitations(paperId, limit = 1000) {
    const citations = [];
    let offset = 0;
    const batchSize = 100; // API limit per request
    
    while (offset < limit) {
      const path = `/graph/v1/paper/${paperId}/citations?fields=paperId,title,authors,year,citationCount,journal,externalIds&limit=${Math.min(batchSize, limit - offset)}&offset=${offset}`;
      
      try {
        // Check cache first to determine if we need rate limiting
        const cachedResult = this.cache.get(path);
        const isCached = cachedResult !== null;
        
        // Only rate limit for non-cached requests
        await this.rateLimit(isCached);
        ...
        // Extract the citing papers from the response
        const citingPapers = result.data.data.map(item => item.citingPaper);
        citations.push(...citingPapers);
        
        // Check if we've retrieved all available citations
        if (citingPapers.length < batchSize) {
          break;
        }
        
        offset += citingPapers.length;
        
        if (!this.quiet && offset < limit && citingPapers.length === batchSize) {
          console.log(`    Fetched ${offset} citations, continuing...`);
        }
      } catch (error) {
        if (!this.quiet) {
          console.warn(`    Error fetching citations at offset ${offset}: ${error.message}`);
        }
        break;
      }
    }
