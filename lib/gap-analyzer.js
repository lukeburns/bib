...
class GapAnalyzer {
   ...
   */
  async getReferencesForUsedCitations(limitPapers = null) {
    let usedCitations = this.citationAnalyzer.getUsedCitationsList();
    
    if (limitPapers && limitPapers > 0) {
      usedCitations = usedCitations.slice(0, limitPapers);
    ...
    console.log(`Analyzing ${usedCitations.length} cited papers (1 request per second)...`);
   ...
   */
  generateBibTeX(reference, suggestedKey) {
    ...
    entry += `  note={Citation count: ${reference.citationCount || 'N/A'}}\n`;
    entry += '}';
  ...
  async findGaps(options = {}) {
    ...
    const referencesMap = await this.getReferencesForUsedCitations(limitPapers);
