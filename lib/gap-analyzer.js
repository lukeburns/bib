...
class GapAnalyzer {
  ...
  generateBibTeX(reference, suggestedKey, citedBy = null) {
    ...
    // Add DOI from either direct field or externalIds
    const doi = reference.doi || (reference.externalIds && reference.externalIds.DOI);
    if (doi) {
      entry += `  doi={${doi}},\n`;
    ...
    if (reference.externalIds && reference.externalIds.ArXiv) {
      if (!doi) { // Only add URL if we didn't already add DOI URL
        entry += `  url={https://arxiv.org/abs/${reference.externalIds.ArXiv}},\n`;
      }
      entry += `  eprint={${reference.externalIds.ArXiv}},\n`;
